...
function CreateOrder(props) {
    ...
    const confirmProduct = async () =>{
        ...
        let total = temp.length != 0 ? temp.reduce((s,c)=>{
            return s+ (c.quantity*c.idProduct.price)
        },0):null
        let data = await getApi(`/user/box?paymentId=${paymentId}&PayerID=${PayerID}&total=${total}`);
        console.log(data)
    ...
    const buyProduct = async () => {
        try {
            ...
            // let name = temp[0].idProduct.productName
            let total = temp.length != 0 ? temp.reduce((s,c)=>{
                return s+ (c.quantity*c.idProduct.price)
            },0):null
            // let prices = temp[0].idProduct.price
            // let quan = temp[0].quantity
            let item  = temp.map((val) => {
                return {
                    name: val.idProduct.productName,
                    sku: 0,
                    price: val.idProduct.price,
                    currency: "USD",
            ...
            const { data } = await postApi('/user/sandbox',{
            //    name,
               total,
            //    prices,
            //    quan,
            ...
            <div className="container_">
                <div className="payment-list">
                    <div className="payment-top">
                     ...
                     {!check && <div className="payment-bottom">
                        <button
